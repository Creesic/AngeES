...
TEST(ScriptCompilerTests, CompilesRunEntryPoint) {
    // Verifies the backported 1.14 run() entry-point node resolves.
    es_script::Compiler compiler;
    compiler.initialize();

    const bool compiled = compiler.compile("assets/test_run_engine.mr");
    if (!compiled) {
        std::ifstream errorLog("error_log.log");
        std::stringstream errorText;
        errorText << errorLog.rdbuf();
